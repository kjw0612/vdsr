...
While SRCNN successfully introduced a deep learning technique into the super-resolution (SR) problem, we find its limitations in three aspects: first, the network only works for single-scale; second, training converges slowly; third, it relies on context of small image regions.

 In this work, we propose a new method to resolve the aforementioned issues.
...
\textbf{Contributions} In summary, in this work, we propose a new CNN-based SISR method that efficiently solves the three challenging problems: scale, convergence and context. Our method can be used to super-resolve an image very clean and sharp to \textcolor{red}{multiple scales} (Figure \ref{fig:fig1}). Our algorithm demonstrates the state-of-the-art quality for all benchmarked scale factors with this single model.
